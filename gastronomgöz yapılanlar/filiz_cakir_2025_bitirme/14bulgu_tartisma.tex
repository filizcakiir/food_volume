...
\begin{itemize}
    \item \textbf{İşlemci:} Apple M2 (8-core CPU, 10-core GPU)
...
\textbf{Model Eğitim Ortamı:}
\begin{itemize}
    \item \textbf{GPU:} NVIDIA RTX 3060 (12 GB VRAM)
    \item \textbf{Eğitim Süresi:} 200 epoch
\end{itemize}
...
\subsubsection{U²-Net Segmentasyon Modeli}

U²-Net modeli, yemek görüntülerinden arka plan ayrıştırması için entegre edilmiştir. Segmentasyon kalitesi, Intersection over Union (IoU) metriği ile değerlendirilebilir \cite{rezatofighi2019giou}:

\begin{equation}
\text{IoU} = \frac{\text{Area of Overlap}}{\text{Area of Union}} = \frac{TP}{TP + FP + FN}
\end{equation}

\textbf{U²-Net Model Özellikleri:}
\begin{itemize}
...
U²-Net modeli, literatürde semantik segmentasyon görevlerinde yüksek performans gösterdiği raporlanmıştır \cite{qin2020u2net}. Yemek segmentasyonu için beklenen IoU değerleri:
\begin{itemize}
...
\begin{itemize}
    ...
    \item \textbf{İşlem:} Image upload → ResNet50 classification → U²-Net segmentation → Weight estimation → Calorie calculation
...
\begin{itemize}
    \item \textbf{Model Dosyaları:} ResNet50 (191 MB), U²-Net (4.7 MB)
...
MiDaS modeli sisteme entegre edilmiş ve ağırlık tahmin algoritmasında destekleyici veri olarak kullanılmaktadır.
...
\begin{enumerate}
    ...
    \item \textbf{U²-Net Segmentasyon:} Lightweight U²-Net modeli (4.7MB) ile hızlı ve etkili segmentasyon. Ticari uygulamaların çoğu temel segmentasyon veya segmentasyon olmadan çalışmaktadır.
